...
public class State implements Serializable {
	...
	/**
	 * Produces a compact string to represent the state. The string contains the
	 * following elements separated by a "-": the board, the phase, the number
	 * of checkers of both colors on the board and the number of checkers of
	 * both colors in the players' hands. The board is represented as 24
	 * character (O for empty, W for White, B for Black; they written in line,
	 * top bottom). The phase is represented as "First", "Second" or "Final".
	 * The checkers on the board are represented as 2 numbers separated by a
	 * comma (white first). Likewise the number of checkers in the hands.
	 * Example: "OBWOOOOOWWBBOBWOOOOOWWBB-First-6,6-3,3"
	 * 
	 * @return a short string that represent the state.
	 */
	public String toCompactString() {
		...
		// board
		result.append("" + board.get("a7") + board.get("d7") + board.get("g7") + board.get("b6") + board.get("d6")
				+ board.get("f6") + board.get("c5") + board.get("d5") + board.get("e5") + board.get("a4")
				+ board.get("b4") + board.get("c4") + board.get("e4") + board.get("f4") + board.get("g4")
				+ board.get("c3") + board.get("d3") + board.get("e3") + board.get("b2") + board.get("d2")
				+ board.get("f2") + board.get("a1") + board.get("d1") + board.get("g1"));
		result.append("-");
		// phase
		result.append(currentPhase.toString());
		result.append("-");
		// # of checkers
		result.append(whiteCheckers + "," + blackCheckers);
		result.append("-");
		result.append(whiteCheckersOnBoard + "," + blackCheckersOnBoard);
		return result.toString();
